...
//$total_amnt=$tot+$tot2;
	...
	$empemail = isset($_POST['emp_email']) ? $_POST['emp_email'] : ''; // Set a default value if the key is not set
	 ...
	 $noofrecordsquery=mysqli_query($link,"select count(1) as noofrecords from `add_goabill1` where wcc_num ='$wcc_num' or service_no ='$service_no' ") or die(mysqli_error($link));
	$noofrecordsqueryfetch=mysqli_fetch_array($noofrecordsquery);
	$noofrecords=$noofrecordsqueryfetch['noofrecords'];
	 if($noofrecords==0)
	{
	
	$sq=mysqli_query($link,"INSERT INTO `add_goabill1`(`date`, `service_no`, `po_no`, `po_date`, `site_name`, `district`, `indus_id`, 
...
$query = "INSERT INTO add_goabill ( `service_no`, `item_code`, `price`, `qty`, `tax_amnt`, `gst_amnt`, `sgst`, `cgst`,`date`,`temp_type`,`uom`,`item_desc`,`sgstamount`,`cgstamount`,`sno`,`id1`,`hsnno`,`sacno`) 
	VALUES 
...
//$total_amnt=$tot+$tot2;
	...
	if($bar_code!=''){
	echo	$v="update `add_goabill1` set `date`='$date', `service_no`='$service_no', `po_no`='$po_no', `po_date`='$po_date', `site_name`='$site_name',
	...
	`total_cgst`='$totcgst',`total_gst`='$tgsttotal',qr_code='$bar_code',`bill_status`='Submited',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no',`type_work`='$type_work',pcw='$pcw' where id='$id'";
	
	$sq=mysqli_query($link,$v) or die(mysqli_error($link));
	
	} else {
		 $v1="update `add_goabill1` set `date`='$date', `service_no`='$service_no', `po_no`='$po_no', `po_date`='$po_date', `site_name`='$site_name',
	 `district`='$district', `indus_id`='$indus_id', 
	`req_ref`='$req_ref', `seeking_id`='$seeking_id', `state`='$state', `seeking_opt`='$seeking_opt', `rfaid_date`='$rfaid_date',
	 `allcoation_date`='$allcoation_date', 
	`wcc_num`='$wcc_num', `wcc_rec_num`='$wcc_rec_num', `total_amnt`='$tot',`total_sgst`='$sgsttotal',
	`total_cgst`='$totcgst',`total_gst`='$tgsttotal',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no',`type_work`='$type_work',pcw='$pcw' where id='$id'";
...
$id=$_POST['id'];
   ...
   if($id1!='' and $id2!=''){
	  $query = "update add_goabill set `service_no`='$service_no', `item_code`='$item_code',item_desc='".addslashes($item_desc)."', `price`='$price',uom='$uom',
...
$bar_code=$_POST['bar_code'];
	...
	$payment_doc_date=$_POST['payment_doc_date'];
	$payment_doc_no=$_POST['payment_doc_no'];
$v="update `add_goabill1` set qr_code='$bar_code',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no' where id='$id'";
