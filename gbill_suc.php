...
//$total_amnt=$tot+$tot2;
	...
	$empemail = isset($_POST['emp_email']) ? $_POST['emp_email'] : '';
	 	$noofrecordsquery=mysqli_query($link,"select count(1) as noofrecords from `add_gbill1` where wcc_num ='$wcc_num' or service_no ='$service_no' ") or die(mysqli_error($link));
	$noofrecordsqueryfetch=mysqli_fetch_array($noofrecordsquery);
	$noofrecords=$noofrecordsqueryfetch['noofrecords'];
	 if($noofrecords==0)
	{
	
	$sq=mysqli_query($link,"INSERT INTO `add_gbill1`(`date`, `service_no`, `po_no`, `po_date`, `site_name`, `district`, `indus_id`, 
...
$query = "INSERT INTO add_gbill ( `service_no`, `item_code`, `price`, `qty`, `tax_amnt`, `gst_amnt`, `sgst`, `cgst`,`date`,`temp_type`,`uom`,`item_desc`,`sgstamount`,`cgstamount`,`sno`,`id1`,`hsnno`,`sacno`) 
	VALUES 
...
//$total_amnt=$tot+$tot2;
	...
	if($bar_code!=''){
	 echo	$v="update `add_gbill1` set `date`='$date', `service_no`='$service_no', `po_no`='$po_no', `po_date`='$po_date', `site_name`='$site_name',
	 `district`='$district', `indus_id`='$indus_id', `ackno`='$ackno', `ackdt`='$ackdt',
	...
	$sq=mysqli_query($link,$v);
		 ...
		 $v1="update `add_gbill1` set `date`='$date', `service_no`='$service_no', `po_no`='$po_no', `po_date`='$po_date', `site_name`='$site_name',
	 `district`='$district', `indus_id`='$indus_id', `ackno`='$ackno', `ackdt`='$ackdt',
	`req_ref`='$req_ref', `seeking_id`='$seeking_id', `state`='$state', `seeking_opt`='$seeking_opt', `rfaid_date`='$rfaid_date',
	 `allcoation_date`='$allcoation_date', 
	`wcc_num`='$wcc_num', `wcc_rec_num`='$wcc_rec_num', `total_amnt`='$tot',`total_sgst`='$sgsttotal',
...
$id=$_POST['id'];
   ...
   if($id1!='' and $id2!=''){
	  $query = "update add_gbill set `service_no`='$service_no', `item_code`='$item_code',item_desc='".addslashes($item_desc)."', `price`='$price',uom='$uom',
...
$bar_code=$_POST['bar_code'];
	...
	$payment_doc_no=$_POST['payment_doc_no'];
		$ackno=$_POST['ackno'];
	$ackdt=$_POST['ackdt'];
echo $v="update `add_gbill1` set qr_code='$bar_code',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no', `bill_status`='Submited',`ackno`='$ackno', `ackdt`='$ackdt'  where id='$id'";
