...
//$total_amnt=$tot+$tot2;
	...
	//  $empemail=$tr['emp_email'];
	 $noofrecordsquery=mysqli_query($link,"select count(1) as noofrecords from `add_apbill1` where wcc_num ='$wcc_num' or service_no ='$service_no' ") or die(mysqli_error($link));
	$noofrecordsqueryfetch=mysqli_fetch_array($noofrecordsquery);
	$noofrecords=$noofrecordsqueryfetch['noofrecords'];
	 if($noofrecords==0)
	{
	
	$sq=mysqli_query($link,"INSERT INTO `add_apbill1`(`date`, `service_no`, `po_no`, `po_date`, `site_name`, `district`, `indus_id`, 
...
//$total_amnt=$tot+$tot2;
	...
	$ackdt=$_POST['ackdt'];
	if($bar_code!=''){
	echo	$v="update `add_apbill1` set `date`='$date', `service_no`='$service_no', `po_no`='$po_no', `po_date`='$po_date', `site_name`='$site_name',
...
$bar_code=$_POST['bar_code'];
	...
	$payment_doc_no=$_POST['payment_doc_no'];
		$ackno=$_POST['ackno'];
	$ackdt=$_POST['ackdt'];
$v="update `add_apbill1` set qr_code='$bar_code',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no',ackno='$ackno',ackdt='$ackdt' where id='$id'";
