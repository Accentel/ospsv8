...
if(isset($_POST['submit'])){
    ...
    if(in_array($_FILES["file"]["type"],$mimes)){
        ...
        $insertQuery = "INSERT INTO gubill (location, cat, user) VALUES ('$lname', '$cat', '$user')";
        $insertResult = mysqli_query($link, $insertQuery);
        $rid = mysqli_insert_id($link);

        $uploadFilePath = 'uploadbilling/gj/'.basename($_FILES['file']['name']);
        ...
        for ($i = 0; $i < $totalSheet; $i++) {
            ...
            foreach ($sheet->getRowIterator() as $row) {
                ...
                if($line != "Line" and $line !=""){
                    $query = "INSERT INTO gubill1 (line, itemdesc, uom, price, qty, amount, id1)
        ...
        echo "<script>alert('Successfully Registered ');window.location.href='gbill1.php?id=$rid';</script>";
    } else {
